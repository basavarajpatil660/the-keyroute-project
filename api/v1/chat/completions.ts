...
// Edge runtime is required here — it's the only Vercel runtime that lets us
// pipe an upstream fetch() Response body straight through to the client as
// a real SSE stream, instead of buffering the whole response in memory.
export const config = { runtime: 'edge' }
export const preferredRegion = 'bom1' // Mumbai — matches Supabase project region (ap-south-1)
...
async function logUsage(supabase: ReturnType<typeof createClient<any>>, a: LogUsageArgs) {
  try {
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (body.stream) {
    const usageTransform = createUsageCapturingTransform(({ promptTokens, completionTokens, estimated }) => {
      ...
      // — at that point Vercel may consider the request "done" and tear
      // down the function before a fire-and-forget fetch() finishes. 
      // waitUntil() explicitly tells the platform to keep this invocation
  ...
  waitUntil(logUsage(supabase, {
    ...
    errorMessage: upstreamResponse.ok ? null : (parsed?.error?.message || 'Upstream returned an error status'),
