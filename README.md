...
> **Note:** "Keyroute" is also the name of an unrelated travel-booking app and a separate community wireless network project. This is **The Keyroute Project** — a self-hosted, open-source AI API gateway built on Supabase Edge Functions. If you're looking for a self-hosted alternative to LiteLLM, Portkey, or OpenRouter that keeps your provider keys inside your own infrastructure, you're in the right place.
...
**How this compares to specific self-hosted tools:** unlike LiteLLM or self-hosted Portkey, which require you to provision and run a separate server or container, Keyroute's gateway logic runs as a Supabase Edge Function — there's no separate process to keep alive, patch, or pay to host. If you already have a Supabase project, there's no additional infrastructure at all.
...
<details>
<summary><strong>Is there a self-hosted alternative to LiteLLM or Portkey that runs on Supabase specifically?</strong></summary>
<br>
Yes — that's exactly what this project is. Instead of running a separate server or container the way LiteLLM and self-hosted Portkey do, the entire gateway runs as a Supabase Edge Function inside a Supabase project you already control.
</details>

<details>
<summary><strong>Is Keyroute the same as the Keyroute travel app or keyroute.net?</strong></summary>
<br>
No — those are unrelated projects that happen to share a similar name. This repository, The Keyroute Project, is a self-hosted AI API gateway built on Supabase and has no connection to any travel-booking service or wireless network project.
</details>

<details>
<summary><strong>Can I run a free self-hosted AI gateway for OpenAI, Groq, and Gemini without a separate server?</strong></summary>
<br>
Yes — that's the core design goal here. The gateway runs as a Supabase Edge Function, so there's no separate server or container to provision, and no cost beyond staying within Supabase's free tier.
</details>
